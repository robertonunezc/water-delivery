...
class ClientBillingInheritanceTestCase(TestCase):
    ...
    def test_branch_inherits_billing_data_from_corporate(self):
        ...
        effective_data = self.branch.billing_info.effective.data
    ...
    def test_branch_inherits_billing_address_from_corporate(self):
        ...
        effective_address = self.branch.billing_info.effective.address
    ...
    def test_branch_uses_own_billing_data_when_available(self):
        ...
        # Enable override so branch can use its own billing information
        self.branch.billing_override_enabled = True
        self.branch.save(update_fields=["billing_override_enabled"])

        # Clear cached billing_info so it reflects the updated override flag
        if 'billing_info' in self.branch.__dict__:
            del self.branch.__dict__['billing_info']

        effective_data = self.branch.billing_info.effective.data
        self.assertTrue(self.branch.billing_override_enabled)
        self.assertEqual(effective_data.rfc, "BRANCH123456XYZ")
        self.assertEqual(effective_data.razon_social, "Sucursal SA de CV")

    def test_branch_own_billing_ignored_without_override(self):
        """Branch own billing should not be effective when override is disabled"""
        ...
        # Still inherits from corporate because override remains disabled
        self.assertFalse(self.branch.billing_override_enabled)
        self.assertEqual(effective_data.rfc, "CORP123456ABC")
        self.assertEqual(effective_data.razon_social, "Corporativo SA de CV")
