...
from .forms import ProductForm, ProductCategoryForm, ProductClientPriceFormSet
from .services import delete_product_category, ensure_product_for_all_clients
...
def _category_queryset() -> QuerySet:
...
@user_passes_test(_is_admin_user)
def list_categories_admin(request: HttpRequest) -> HttpResponse:
    search_query = request.GET.get('search', '').strip()
    queryset = _category_queryset()

    if search_query:
        queryset = queryset.filter(name__icontains=search_query).distinct()
...
@user_passes_test(_is_admin_user)
def create_category_admin(request: HttpRequest) -> HttpResponse:
    if request.method == 'POST':
        form = ProductCategoryForm(request.POST)
        if form.is_valid():
            category = form.save()
            messages.success(request, 'Categoría creada exitosamente.')
            return redirect('admin_edit_category', pk=category.pk)
    else:
...
@user_passes_test(_is_admin_user)
def edit_category_admin(request: HttpRequest, pk: int) -> HttpResponse:
...
@user_passes_test(_is_admin_user)
@require_POST
def delete_category_admin(request: HttpRequest, pk: int) -> HttpResponse:
    category = get_object_or_404(ProductCategory.objects, pk=pk)
    result = delete_product_category(category)
    ...
    messages.error(
        request,
        f"No se puede eliminar la categoría porque tiene {result['product_count']} producto(s) asociado(s).",
    )
    return redirect('admin_edit_category', pk=category.pk)
