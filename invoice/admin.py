	...
	def __init__(self, *args, **kwargs):
		...
		if invoice and 'order' in self.fields:
			...
			self.fields['order'].queryset = get_invoiceable_orders_for_client(
			  ...
			  # No invoice context available (e.g. empty form template for new inline rows).
			  # Return an empty queryset so no options are shown server-side; the inline
			  # JS will populate the select once a client is selected.
